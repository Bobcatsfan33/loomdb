...
| **AT-047** | Sleep the tenant, **wipe the disk**, wake elsewhere. Identical results, and the branch names come back with the data. **Latency:** same-runner p99 13 ms (inside target). Wide-area, intercontinental: serial p99 ~1000 ms; **batched via substrate get_batch p99 808 ms** — improved but STILL over 250 ms. Breakdown: fault set mean 2 pages; get_batch(pages) ~450 ms (≈2 RTT — cold connection pool pays fresh handshakes) + serial overlay-manifest chain ~310 ms (get_batch doesn't touch it). Remaining levers: warm keep-alive pool sized to width + manifest-chain batching. Correctness green; airgap deployments don't wake from object storage. See README known limits. | `at_047_sleep_wipe_wake_and_the_branches_come_back` (correctness), `wake_latency::at_047_wake_latency_over_object_storage` (latency) |
