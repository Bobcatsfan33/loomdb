...
| **AT-047** | Sleep the tenant, **wipe the disk**, wake elsewhere. Identical results, and the branch names come back with the data. **Latency (measured both ways):** same-runner p99 13 ms (inside target); **wide-area, intercontinental, p99 1020 ms — ~4× OVER the 250 ms target** (wake is O(pages-touched) serial faults × ~230 ms RTT; needs v0.3 fault-batching to meet it wide-area; airgap deployments don't wake from object storage). Correctness green; the latency target is met same-runner, NOT wide-area — see README known limits. | `at_047_sleep_wipe_wake_and_the_branches_come_back` (correctness), `wake_latency::at_047_wake_latency_over_object_storage` (latency) |
