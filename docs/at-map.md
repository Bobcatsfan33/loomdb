...
| **AT-047** | Sleep the tenant, **wipe the disk**, wake elsewhere. Identical results, and the branch names come back with the data. **Latency (RTT-framed; the algorithm is optimal — the rest is a function of the connection pool):** same-runner p99 13 ms (inside target, not wide-area). Wide-area intercontinental (Sydney, 50 samples, RTT ≈ 226 ms): **cold first-ever ~4 RTT** (p50 ~920 ms, serial overlay-chain pointer-chase); **hot re-wake with the learned warm set (substrate ≥ v1.4.2)** — records the manifest ids + page ids a session faults, `sleep()` carries them, wake **hydrates them in one awaited concurrent batch**. PROVEN optimal: after hydrate the read faults **0** objects, so the whole cost is the concurrent hydrate, a pure function of the pool: **cold pool ~2.3 RTT** (p50 ~634 ms — HTTP/1.1 S3, N concurrent GETs pay N TLS handshakes); **warm pool ~1.03 RTT** (p50 **232 ms** / p99 278 ms — reused keep-alive connections, warm read essentially free). So on a warm pool hot re-wake ≈ 1×RTT: **clears 250 ms at p50 even to Sydney**, p99 (278 ms) just over at that extreme, comfortably under for nearer links. Warm pool = continuous load or a maintained keep-alive pool (the remaining transport step, infra not algorithm). Cold stays ~4 RTT. Airgap deployments don't wake from object storage. See README known limits. | `at_047_sleep_wipe_wake_and_the_branches_come_back` (correctness), `wake_latency::at_047_hot_vs_cold_rewake_over_object_storage` + `at_047_wake_latency_over_object_storage` (latency) |
