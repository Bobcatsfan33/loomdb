...
| **AT-047** | Sleep the tenant, **wipe the disk**, wake elsewhere. Identical results, and the branch names come back with the data. **Latency:** wake → first read is p50 12 ms / p99 13 ms over a real S3 endpoint (same-runner MinIO) — inside the < 250 ms target over the object-storage protocol; the wide-area p99 over a genuinely remote bucket is the one open figure (see README known limits). | `at_047_sleep_wipe_wake_and_the_branches_come_back` (correctness), `wake_latency::at_047_wake_latency_over_object_storage` (latency) |
