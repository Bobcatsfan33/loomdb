...
| **AT-047** | Sleep the tenant, **wipe the disk**, wake elsewhere. Identical results, and the branch names come back with the data. **Latency (RTT-framed; the algorithm is optimal — the rest is a function of the connection pool):** same-runner p99 13 ms (inside target, not wide-area). Wide-area intercontinental (Sydney, 50 samples, RTT ≈ 226 ms): **cold first-ever ~4 RTT** (p50 ~920 ms, serial overlay-chain pointer-chase); **hot re-wake with the learned warm set (substrate ≥ v1.4.2)** — records the manifest ids + page ids a session faults, `sleep()` carries them, wake **hydrates them in one awaited concurrent batch**. PROVEN optimal: after hydrate the read faults **0** objects, so the whole cost is the concurrent hydrate, a pure function of the pool: **cold pool ~2.3 RTT** (p50 ~634 ms — HTTP/1.1 S3, N concurrent GETs pay N TLS handshakes); **warm pool ~1.03 RTT** (p50 **232 ms** / p99 278 ms — reused keep-alive connections, warm read essentially free). So on a warm pool hot re-wake ≈ 1×RTT. **The warm pool is now guaranteed, not hoped for (v1.5.0):** `substrate::WarmPool` holds `min_idle` keep-alive connections open; measured wide-area, a cold-start re-wake drops from **2.88 RTT (no pool, p50 499 ms) to ~1 RTT at the median (pool, p50 179 ms)**, though the **p99 stays ~2 RTT (345 ms)** at this extreme distance. Scoped with the same median-vs-tail honesty: **~1 RTT at the median up to min_idle connections**; the p99 tail can still pay a second RTT, and bursts beyond min_idle pay handshakes — which the in-region topology below removes (even ~2 RTT clears 250 ms at single-digit-ms RTT). **The remaining limit is TOPOLOGY, not code:** wake ≈ 1 RTT to your object store, and whether that clears 250 ms is distance — the SLA is "wake ≈ 1 RTT to your object store" and the recommendation is to **co-locate the object tier in-region** (single-digit-to-low-tens-of-ms RTT clears 250 ms p99 with margin). The Sydney numbers are a **deliberately extreme worst case** (opposite sides of the planet): median clears 250 ms, p99 (~345 ms) does not — geography, not an engine gap. A **regional cache tier** (future feature, not built) would keep it ~1 RTT for a global fleet. Cold first-ever stays ~4 RTT. Airgap deployments don't wake from object storage. See README known limits. | `at_047_sleep_wipe_wake_and_the_branches_come_back` (correctness), `wake_latency::at_047_hot_vs_cold_rewake_over_object_storage` + `at_047_warm_pool_delivers_one_rtt_on_cold_start` (latency) |
...
| **AT-040** | **Branch-aware indexes — isolation is structural.** A sibling branch's write is never retrieved, because index entries live in the branch's own tree and a sibling has a different head manifest. Verified by a model oracle at 3,000 randomized runs: what a branch retrieves equals what the model says is visible there, **and never a sibling's fact**. Isolation is checked on **both retrieval surfaces** — the ANN graph search and the exact scan (`search_ann` and `search_scan`) — since each is a distinct path that reads the branch fork. | `at_040_a_siblings_write_is_never_retrieved`, `retrieval_sees_exactly_the_branch_and_never_a_sibling` (oracle), `ann_search_never_returns_a_siblings_vectors` (both surfaces) |
...
**The index is LIVE (v0.4, slice 2c resolved on the number).** The write-path-placement question —
inline vs. background compaction — was decided by measurement (`benches/ann_amplification.rs`): an inline
ANN insert added growing write amplification (~1.7–2.2×) and, disqualifyingly, ~220 ms of per-write
latency that grows with the graph, on the AT-045-certified write path. So **compaction**: an indexed
write appends its vector to a reserved in-branch **buffer** (≈1× baseline, same commit); `search_ann`
**unions** the graph with a bounded buffer brute-scan, so a freshly-written vector is searchable
immediately (**0 staleness**); a background **fold** (`Loom::ann_fold`) moves the buffer into the graph
off the write path, published by a **compare-and-set** on the head so it never stalls or clobbers a live
write. Gates (`tests/compaction.rs`, `tests/crash.rs`): the handoff never drops or double-counts a vector;
recall holds on the union; AT-040 isolation holds for the unfolded buffer; **AT-045 over the fold** — a
crash at any byte leaves every acked vector in the buffer *or* the graph, never neither; and a
wake-piece-2-class **concurrency gate** — the fold racing appends and union-searches loses no item,
double-indexes none, and never deadlocks. Held-for-review confirmed: the write path stays ~1× baseline
(the buffer append is one small entry on a ~220 KB commit), and the union scan is O(buffer) (prefix-ranged
`Tree::scan_prefix`), bounded by the fold threshold.
