...
| **AT-047** | Sleep the tenant, **wipe the disk**, wake elsewhere. Identical results, and the branch names come back with the data. **Latency (RTT-framed, the algorithm is now optimal — the rest is transport):** same-runner p99 13 ms (inside target, not wide-area). Wide-area intercontinental (Sydney, 50 samples, 1 warm GET ≈ 160–230 ms): **cold first-ever ~4 RTT** (p50 ~920 ms, serial overlay-chain pointer-chase); **hot re-wake with the learned warm set (substrate ≥ v1.4.2) ~2.3 RTT** (p50 ~634 ms / p99 ~696 ms) — the warm set records the manifest ids + page ids a session faults, `sleep()` carries them, wake **hydrates them in one awaited concurrent batch**. PROVEN optimal: after hydrate the read faults **0** objects, so the whole remaining cost is the concurrent hydrate itself — **pure transport** (HTTP/1.1 S3 ⇒ N concurrent GETs pay N TLS handshakes ≈ 2.3 RTT vs the ~1 RTT the batch should cost). Remaining v0.3 lever is a **keep-alive pool pre-warmed to the concurrency width** (→ ~1 RTT hydrate). Clears 250 ms wherever `RTT + ~110 ms < 250 ms` (RTT ≲ 130 ms, regional/near-continental); NOT at extreme intercontinental distance (Sydney ~270–340 ms even at 1-RTT hydrate). Cold stays ~4 RTT. Airgap deployments don't wake from object storage. See README known limits. | `at_047_sleep_wipe_wake_and_the_branches_come_back` (correctness), `wake_latency::at_047_hot_vs_cold_rewake_over_object_storage` + `at_047_wake_latency_over_object_storage` (latency) |
